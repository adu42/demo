...
namespace Ado\Spark\Providers;

use Ado\Spark\Spark;
use Ado\Spark\TokenGuard;
use Illuminate\Support\Facades\Auth;
use Intervention\Image\ImageManager;
use Illuminate\Support\Facades\Route;
use Illuminate\Support\ServiceProvider;
use Illuminate\Support\Facades\Validator;
use Ado\Spark\Validation\StateValidator;
use Ado\Spark\Validation\VatIdValidator;
use Ado\Spark\Validation\CountryValidator;
use Ado\Spark\Console\Commands\InstallCommand;
use Ado\Spark\Console\Commands\UpdateCommand;
use Ado\Spark\Console\Commands\VersionCommand;
use Ado\Spark\Console\Commands\UpdateViewsCommand;
use Ado\Spark\Console\Commands\StorePerformanceIndicatorsCommand;
...
class SparkServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        if (! class_exists('Spark')) {
            class_alias('Ado\Spark\Spark', 'Spark');
        }
    ...
    protected function registerServices()
    {
        ...
        foreach ($services as $key => $value) {
            $this->app->singleton('Ado\Spark\\'.$key, 'Ado\Spark\\'.$value);
        }
    }
    ...
    {
        $this->app->when('Ado\Spark\Services\Security\Authy')
                ->needs('$key')
                ->give(function () {
    ...
    private function registerApiTokenRepository()
    {
        $concrete = class_exists('Laravel\Passport\Passport')
                        ? 'Ado\Spark\Repositories\PassportTokenRepository'
                        : 'Ado\Spark\Repositories\TokenRepository';

        $this->app->singleton('Ado\Spark\Contracts\Repositories\TokenRepository', $concrete);
    }
}
